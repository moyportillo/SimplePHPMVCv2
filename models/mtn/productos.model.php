...
function getProductoById($codprd){
    $sqlstr = "SELECT * from productos where codprd = %d;";
    return obtenerUnRegistro(sprintf($sqlstr, $codprd));
}


function addNewProducto($dscprd,$sdscprd,$skuprd,$bcdprd,$stkprd, $typprd,$prcprd,$urlprd,$estprd){
    $inssql = " INSERT INTO productos (`dscprd`,`sdscprd`,`ldscprd`,`skuprd`,`bcdprd`,`stkprd`,`typprd`,`prcprd`,`urlprd`,`urlthbprd`,`estprd`)
    VALUES ('%s', '%s', NULL, '%s', '%s', %d, '%s', %d, '%s', '', '%s');";
    return ejecutarNonQuery(sprintf($inssql,$dscprd,$sdscprd,$skuprd,$bcdprd,$stkprd,$typprd,$prcprd,$urlprd,$estprd));
}
...
function updateProducto($dscprd,$sdscprd,$skuprd,$bcdprd,$stkprd, $typprd,$prcprd,$urlprd,$estprd,$codprd){
    $updsql = "UPDATE `productos` SET `dscprd`= '%s', `sdscprd`='%s', `ldscprd`=NULL, `skuprd`='%s', `bcdprd`='%s', `stkprd` ='%d', `typprd`='%s', `prcprd` ='%d', `urlprd`='%s', `urlthbprd`='', `estprd`='%s' WHERE `codprd` = '%d';";
...
function deleteProducto($codprd){
    return ejecutarnonQuery(sprintf("DELETE from productos where codprd = %d;", $codprd));
}
